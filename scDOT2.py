...
import math
import ot
from utils2 import pp, concatenate, find_index, vstack, cal_mmd, dist_ensemble, H
import time
...
def solve(D, a, b, label_set, beta, w_piror=None, lambda1=0.01, lambda2=0.01, maxIter2=100, tol=1e-2):
    ...
    x, w, M = _solve(D, a, b, w_piror, maxIter=maxIter2, lambda1 = lambda1, lambda2 = lambda2, beta=beta, tol=tol)
...
def scDOT(loc, ref_name, query_name, lambda1=0.01, lambda2=0.01, threshold=0.9):
    ...
    for i in ref_name:
        ...
        a=pd.read_csv(file_name, header=0, index_col=0).iloc[:,0].values # 原数据集用
        # a=pd.read_csv(file_name, header=0, index_col=False).iloc[:,0].values # 换数据集用
        label_s.append(a)
    ...
    x, w, y_hat, M = solve(D, b_hat, a_hat, label_set, w_piror=w, lambda1=lambda1, lambda2=lambda2, beta=lambda3)
